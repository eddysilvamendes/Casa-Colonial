...
$expected = ['arivel_date', 'departure_date', 'roons', 'adults','name','surmane','email','num_doc','telefone','address','origin_country','nationality','travel_motive','info'];
$required = ['arivel_date', 'departure_date', 'roons', 'adults','name','surname','email','num_doc','telefone','address','origin_country','nationality','travel_motive','info'];
...
if (!$errors && !$missing) {
    ...
    $replacements = [
        'madalena.barreto@primeconsulting.org' =>
            ['#subject#' => 'Reservation Form Casa Colonial',
                '#greeting#' => "Thanks $name, your booking request was received!"]

            $email =>
            ['#subject#' => 'Casa Colonial Reservation Request',
    ...
    try {
        ...
        $message = Swift_Message::newInstance()
         ...
         $image_ilha = $message->embed(Swift_Image::fromPath('img/saovicente.png'));
            $image_logo = $message->embed(Swift_Image::fromPath('img/logo.jpg'));

        // create the first part of the HTML output
        $html = <<<EOT
<head>
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
<title>Casa Colonial</title>
</head>
